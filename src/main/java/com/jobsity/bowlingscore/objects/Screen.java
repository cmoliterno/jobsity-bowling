...
import com.jobsity.bowlingscore.helpers.Helper;
...
public class Screen {
    ...
    public Screen(int index, String pointsFromBall){
        this.index = index;
    ...
    public void calculateTotal(List<Screen> frames){
    ...
    private int previousFrameTotal(List<Screen> frames){
        if(index == 1) return 0;
        
        return frames.get(index - 2).total;
    }
    
    private int getNextTwoBoxes(List<Screen> frames) {
    ...
    private int getNextBox(List<Screen> frames, int actualbox){
